%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Deedy CV/Resume
% XeLaTeX Template
% Version 1.0 (5/5/2014)
%
% This template has been downloaded from:
% http://www.LaTeXTemplates.com
%
% Original author:
% Debarghya Das (http://www.debarghyadas.com)
% With extensive modifications by:
% Vel (vel@latextemplates.com)
%
% License:
% CC BY-NC-SA 3.0 (http://creativecommons.org/licenses/by-nc-sa/3.0/)
%
% Important notes:
% This template needs to be compiled with XeLaTeX.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[a4paper]{deedy-resume} % Use US Letter paper, change to a4paper for A4 

\begin{document}

%----------------------------------------------------------------------------------------
%	TITLE SECTION
%----------------------------------------------------------------------------------------

% \lastupdated % Print the Last Updated text at the top right

\namesection{João}{Pela}{ % Your name
% \urlstyle{same}\url{http://debarghyadas.com} \\ % Your website, LinkedIn profile or other web address
\href{mailto:joaopela@gmail.com}{joaopela@gmail.com} | +44 7919533220 % Your contact information
}

%----------------------------------------------------------------------------------------
%	LEFT COLUMN
%----------------------------------------------------------------------------------------

\begin{minipage}[t]{0.35\textwidth} % The left column takes up 33% of the text width of the page

%------------------------------------------------
% Education
%------------------------------------------------

\section{Education} 

\subsection{\href{https://www.imperial.ac.uk/}{Imperial College London}}

\descript{PhD in Experimental Physics Research}
\location{Expected Dec 2015 | London, UK}

\sectionspace % Some whitespace after the section

\subsection{\href{https://tecnico.ulisboa.pt/en/}{Instituto Superior Técnico}}
\descript{M.S. in Engineering Physics and Technology}
\location{Oct 2010 | Lisbon, PT}
\location{Final grade: 90\% (Excellent)}

\sectionspace % Some whitespace after the section

\descript{B.S. in Engineering Physics and Technology}
\location{Feb 2009 | Lisbon, PT}
\location{Final grade: 70\% (Good)}

\sectionspace % Some whitespace after the section

%------------------------------------------------



%------------------------------------------------
% Links
%------------------------------------------------
% \section{Links} 
% 
% Github:// \href{https://github.com/deedydas}{\bf deedydas} \\
% LinkedIn:// \href{https://www.linkedin.com/in/debarghyadas}{\bf debarghyadas} \\
% YouTube:// \href{https://www.youtube.com/user/DeedyDash007}{\bf DeedyDash007} \\
% Twitter:// \href{https://twitter.com/debarghya_das}{\bf @debarghya\_das} \\
% Quora:// \href{https://www.quora.com/Debarghya-Das}{\bf Debarghya-Das}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Coursework
%------------------------------------------------

\section{Skills}

\subsection{Graduate}

Data Analysis \\
Data Reduction \\
Real Time Monitoring \\
Machine Learning \\
Statistical Analysis \\
Error Estimation \\

\sectionspace % Some whitespace after the section

%------------------------------------------------

\subsection{Undergraduate}

Data Analysis \\
Instrumentation \\
Programming \\
Simulation \\

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Skills
%------------------------------------------------

\subsection{Programming}

\location{Over 10000 lines:}
C++ \textbullet{} Python \textbullet{} Shell \textbullet{} JavaScript \\
Java \textbullet{} PHP \textbullet{} \textbullet{} \LaTeX\ \textbullet{} VHDL \\ 
\location{Over 1000 lines:}
C \textbullet{} CSS \textbullet{} Assembly \textbullet{} MySQL \\
 Fortran \textbullet{} Mathematica \\
\location{Familiar:}
Matlab \textbullet{} XML

\sectionspace % Some whitespace after the section

\subsection{Languages}

Portuguese: Native \\
English: Bilingual proficiency \\
French: Full professional proficiency \\
Spanish: Professional working proficiency \\

\sectionspace % Some whitespace after the section

\subsection{References} 
Available upon request

\sectionspace % Some whitespace after the section

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the left column
\hfill
%
%----------------------------------------------------------------------------------------
%	RIGHT COLUMN
%----------------------------------------------------------------------------------------
%
\begin{minipage}[t]{0.62\textwidth} % The right column takes up 66% of the text width of the page

%------------------------------------------------
% Experience
%------------------------------------------------

\section{Experience}

\runsubsection{\href{https://www.imperial.ac.uk/}{Imperial College London (IC)}}
\descript{| Ph.D. Student working on the \href{http://cms.web.cern.ch/}{CMS Experiment} at the \href{http://home.cern/topics/large-hadron-collider}{LHC}}

\location{Jan 2012 - Present | London, UK}
\vspace{\topsep} % Hacky fix for awkward extra vertical space
\begin{tightitemize}
\item Developed data analysis methods to isolate signal events in extremely large and noisy datasets.
\item Developed hardware and software algorithms for data reduction while keeping high signal collection efficiency.
\item Analysed and quantified systematic errors associated with the choice of a specific model for interpreting data.
\item Participated in data collection operations as field manager and expert on call.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{\href{http://cern.ch}{European Organization for Nuclear Research (CERN)}}
\descript{| \href{http://cms.web.cern.ch/}{CMS} L1T Data Quality Monitoring Coordinator}

\location{Jul 2012 - Jun 2014 | Geneva, Switzerland}
\begin{tightitemize}
\item Led a group of 5 people with the task of maintaining high operational standards of the CMS Level 1 Trigger system (hardware data reduction system). 
\item Responsible for the real time monitoring of a mission critical system.
\item Responsible for the certification of data quality for physics analysis usage.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------

\runsubsection{\href{http://cern.ch}{European Organization for Nuclear Research (CERN)}}
\descript{| Engineer/Physicist on the \href{http://cms.web.cern.ch/}{CMS Experiment} at the \href{http://home.cern/topics/large-hadron-collider}{LHC}}

\location{Feb 2011 - Dec 2011 | Geneva, Switzerland}
\begin{tightitemize}
\item Designed and implemented a real time monitoring system for a high rate, multi-million input channel hardware data reduction system.
\item Participated in data collection operations as on site expert.
\end{tightitemize}

\sectionspace % Some whitespace after the section

\runsubsection{\href{http://www.lip.pt/}{Laboratory for Instrumentation and Experimental Particle Physics (LIP)}}
\descript{| Junior Researcher on the \href{http://cms.web.cern.ch/}{CMS Experiment} at the \href{http://home.cern/topics/large-hadron-collider}{LHC}}

\location{2007 - 2010 | Lisbon, Portugal}
\begin{tightitemize}
\item Developed data analysis methods to isolate signal events in extremely large and noisy datasets.
\item Responsible for group participation in a CMS collaboration wide exercise to test global data collection and analysis capability.
\item Participated in data collection operations as on site expert.
\end{tightitemize}

\sectionspace % Some whitespace after the section

\runsubsection{\href{http://www.cfn.ist.utl.pt/eng/}{Centre for Nuclear Fusion (CFN)}}
\descript{| Experimental Physist}

\location{2003 - 2004 | Lisbon, Portugal}
\begin{tightitemize}
\item  Designed and tested an electronics board for the slow control of a nuclear fusion reactor.
\end{tightitemize}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Awards
%------------------------------------------------

\section{Awards} 

\begin{tabular}{cl}
2015	& \begin{tabular}[l]{@{}l@{}}IC, Faculty of Natural Science, Prize for Excellence in the Support\\ of Teaching \& Learning\end{tabular}\\
2015    & IC, London, Physics $1^{st}$ Year Laboratories, Best Demonstrator Award \\
2012-15 & IC London, Development of Excellence in Sport Scholarship (Judo) \\
\end{tabular}

\sectionspace % Some whitespace after the section

%------------------------------------------------
% Societies
%------------------------------------------------

\section{Societies} 

\begin{tabular}{rll}
2014-15 & Men's Team Captain   & Imperial College London Judo Club \\
2006-10 & Leader of Activities & NFIST - IST Physics Students Association \\
\end{tabular}

% \sectionspace % Some whitespace after the section

%----------------------------------------------------------------------------------------

\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------
%	SECOND PAGE (EXAMPLE)
%----------------------------------------------------------------------------------------

%\newpage % Start a new page

%\begin{minipage}[t]{0.33\textwidth} % The left column takes up 33% of the text width of the page

%\section{Example Section}

%\end{minipage} % The end of the left column
%\hfill
%\begin{minipage}[t]{0.66\textwidth} % The right column takes up 66% of the text width of the page

%\section{Example Section 2}

%\end{minipage} % The end of the right column

%----------------------------------------------------------------------------------------

\end{document}
